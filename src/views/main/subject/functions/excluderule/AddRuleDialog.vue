...
const { data: usersR } = SubjectService.listUsersService()
const excludeRuleR = ref<ExcludeRule>({})
const { mutateAsync } = SubjectService.addExcludeRuleService()
...
const submitF = async () => {
  ...
  excludeRuleR.value.teacherName = usersR.value!.find(
    (u) => u.id == excludeRuleR.value.userId
  )?.name
  dialogFormVisible.value = false

  const rule: ExcludeRule = JSON.parse(JSON.stringify(excludeRuleR.value))
  await mutateAsync(rule)
    ...
    <el-form :model="excludeRuleR" label-width="60px">
      ...
      <el-form-item label="起止周">
        <div style="display: flex; flex-direction: column">
          <el-radio-group @change="weeksRadioChangeF" v-model="weeksR" style="margin-bottom: 10px">
            <el-radio-button :value="1">上半学期</el-radio-button>
            <el-radio-button :value="2">下半学期</el-radio-button>
            <el-radio-button :value="3">整学期</el-radio-button>
          </el-radio-group>
          <div>
            <el-input-number
      ...
      <el-form-item>
        <el-button @click="submitF" type="success" :disabled="!excludeRuleR.userId">提交</el-button>
      </el-form-item>
    </el-form>
  </el-dialog>
</template>

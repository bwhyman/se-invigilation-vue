...
基于tanstack-query缓存库重构，优化。

#### 2025-07-16

每名监考独立设置日历，避免不同钉钉组教师无法邀请。

#### 2025-04-17

修复获取全部教师监考数量错误。什么时候把请求地址改错误了？！~
...
在复用组件中将传入的响应式数据对象引为本地变量使用，致使组件每次均基于初始化时传入的数据操作！通过直接绑定响应式数据解决。本应避免的低级错误。

#### 2024-09-26

读取课表时移除section中的`分组`行。  
如果专业分配负责人为一人，分配提醒时默认为选中，避免每次都得选择。

#### 2024-08-01

学院/专业均可录入监考信息，但由学院统一分配。

#### 2024-07-31

按HTML5模式设置路由路径。  
终于实现docker nginx的对应配置。原导入自定义配置并自己编写容器内执行函数加载变量方法，会导致设置的try_files参数死循环(不知道为什么)。
使用>1.19版本镜像支持的自带函数整合自定义配置方法成功。

#### 2024-07-30

调整store与loading装饰器顺序。先执行store装饰器，由于命中缓存后无延迟加载数据且不再执行目标方法，则也无需执行loading装饰器。  
调整store缓存中数组类型默认值为空，实现缓存响应的空数组，避免无数据时发送请求。

#### 2024-07-29

更新vue/element-plus等至最新版。

#### 2024-07-27

通过装饰器实现对有较大延迟数据请求业务添加全屏加载框。直接针对耗时的业务处理而非显示组件，既可复用也可避免在各组件的冗余调用。
...
优化监考分配组件。为监考教师添加对应的按钮状态附属数据。监听选择数据长度，通过在一次循环中更新全部按钮状态(`M`)，避免大量计算属性的重复计算(`M * N`)，提高了执行效率。  
子表格组件渲染并执行分配逻辑，父组件监听变化并更新子组件按钮状态，组件间逻辑结构也更清晰。

#### 2024-07-13

优化学院分配组件，分离过于复杂的业务逻辑与组件渲染逻辑。

#### 2024-07-12

专业追加查询/导出任意日期段监考信息，用于截图提醒等。

#### 2024.07.11

支持学院跨专业/不受人数限制的分配监考。  
把全局通用数据在根组件加载时请求并置于store，组件基于store使用。
对于setting/user等确定数据直接基于store使用，按service+store设计总要异步加载过于繁琐。
...
而非在打包时将模块需要的工具函数分割。即，即使仅需使用工具模块中的一个函数，也需要加载整个模块，从而影响渲染速度。
